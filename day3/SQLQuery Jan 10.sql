...
exec CRM_Usp_GetLeads_Backup 1248,null,1,10,null,null,null,null,null,null,null,null,null,null,null,'2024-01-10','2024-01-11','CreatedDate' ,5
CREATE PROC [dbo].[CRM_Usp_GetLeads_Backup]                 
@UserId INT   
...
BEGIN                
 ...
 AS (                
  SELECT ROW_NUMBER() OVER (                
   ...
   ,CAST(FORMAT(cl.CreatedDate, 'MM-dd-yyyy hh:mm:ss tt') AS VARCHAR(30)) AS CreatedDate   
  FROM [dbo].[CRMLead] cl                
  INNER JOIN [dbo].[CRMLeadStatus] cls ON cls.Id = cl.[StatusId]                
  LEFT JOIN [dbo].[CRMLeadMapping] clm ON cl.Id = clm.[LeadId]                
  LEFT JOIN #TempTeam t ON t.UserId = clm.[UserId]                
