--EXEC USP_GetOpportunities 1248, '2023-11-01', '2023-12-30', NULL, NULL, NULL, NULL, 'CloseDate'
...
BEGIN          
 ...
 WHERE                     
  o.IsActive = 1 
  AND (
  (@DateType = 'CreateDate' AND CAST(o.CreateDate AS DATE) >= @FromDate AND CAST(o.CreateDate AS DATE) <= @ToDate) OR
  (@DateType = 'ModifiedDate' AND CAST(o.ModifiedDate AS DATE) >= @FromDate AND CAST(o.ModifiedDate AS DATE) <= @ToDate) OR
  (@DateType = 'CloseDate' AND CAST(o.CloseDate AS DATE) >= @FromDate AND CAST(o.CloseDate AS DATE) <= @ToDate) 
  )
